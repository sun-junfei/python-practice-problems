...
def prune_tree(tree, keys_to_discard):
    ...
    def dfs(tree,  keys_to_discard):
        if tree.key in keys_to_discard:
            return None
        new_tree = Tree(tree.key, tree.value)
        for child in tree.children:
            nxt_child = dfs(child,  keys_to_discard)
            if nxt_child:
                new_tree.children.append(nxt_child)
        return new_tree

    return dfs(tree, keys_to_discard)
...
def do_test_prune_tree(trees_original_expected, tree_name, keys_to_prune):
    ...
    tree_utils.check_tree_equals(
        actual, expected_trees[tree_name], recreate_msg)
